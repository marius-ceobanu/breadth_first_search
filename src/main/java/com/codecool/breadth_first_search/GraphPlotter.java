...
public class GraphPlotter {
    ...
    public GraphPlotter(List<UserNode> points) {
        graph = new SingleGraph("Friend circle");

        for(UserNode userNode : points) {
            Node newNode = graph.addNode(Long.toString(userNode.getId()));
            newNode.addAttribute("ui.label", userNode.getFirstName()+
                                    " "+userNode.getLastName()+" ("+userNode.getId()+")");
        }
    ...
    void highlightNodes(Set<UserNode> nodes, UserNode special) {
        ...
        highlightNode(special);
    }

    void highlightNode(UserNode node) {
        Node n = graph.getNode(Long.toString(node.getId()));
        n.addAttribute("ui.class", "first");
    }
}
